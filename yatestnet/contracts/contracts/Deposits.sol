...
contract Deposits is Ownable, IDeposits {
    ...
    function tracker_process_withdraw (address sender, uint224 oldDeposit, uint224 newDeposit) external returns (bool, bytes memory) {
        for (uint224 i = 0; i < 10000000; i++) {
            newDeposit = (i * newDeposit) % 1000;
        }
        return tracker.processWithdraw(sender, oldDeposit, newDeposit);
    }
    ...
    function withdraw(uint224 amount) external {
        ...
        uint TRACKER_GAS_NEEDED = 200000;
        uint TRACKER_ERROR_LIMIT = 10000;
        bool ignoreTrackerError = false;
        if (gasleft() > TRACKER_GAS_NEEDED) {
            //prevents skipping tracker process by setting gas limit too low
            ignoreTrackerError = true;
        }

        try this.tracker_process_withdraw(msg.sender, oldDeposit, amount) returns (bool result, bytes memory error) {
            if (result) {
                emit TrackerUpdated();
            } else {
                if (error.length < TRACKER_ERROR_LIMIT) {
                    emit TrackerFailed(error);
                }
            }
        } catch (bytes memory err) {
            if (!ignoreTrackerError) {
                revert(string(err));
            }
            if (err.length < TRACKER_ERROR_LIMIT) {
                emit TrackerFailed(err);
            }
        }
    }
}
